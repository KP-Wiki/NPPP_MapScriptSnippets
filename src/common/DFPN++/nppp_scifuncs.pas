...
type
  ...
  public
    ...
    //text
    procedure addText(const aString: nppString);
    ...
    procedure replaceSelection(const aString: nppString);
...
procedure TNPPSciFunctions.replaceSelection(const aString: nppString);
var
  aStr: AnsiString;
begin
  aStr := AnsiString(aString);
  SendMessage(getCurrentSciHandle, SCI_REPLACESEL, 0, LPARAM(PAnsiChar(aStr)));
end;
...
procedure TNPPSciFunctions.addText(const aString: nppString);
var
  aStr: AnsiString;
begin
  aStr := AnsiString(aString);
  SendMessage(getCurrentSciHandle, SCI_ADDTEXT, WPARAM(Length(aStr) + 1),
              LPARAM(PAnsiChar(aStr)));
end;
