...
{$if defined(VER290) or defined(VER300) or defined(VER310) or
     defined(VER320)}
  {$define VER210ORGREATER}
  {$define VER230ORGREATER}
  {$define VER290ORGREATER}
{$endif}
    ...
    case FDataType of
      stObject:
        ...
        begin
          k := 0;
          Append(TOK_CBL, 1);
          if indent then _indent(4, false);
...
{$ENDIF}
  ...
  var
    o: ISuperObject;
{$ifdef VER290ORGREATER}
    tdguid: TGuid;
  begin
    tdguid := GetTypeData(TypeInfo).Guid;

    if CompareMem(@tdguid, @soguid, SizeOf(TGUID)) then
{$else VER290ORGREATER}
  begin
    if CompareMem(@GetTypeData(TypeInfo).Guid, @soguid, SizeOf(TGUID)) then
{$endif VER290ORGREATER}
    begin
      if obj <> nil then
